...
|---|---|---|---|
|id | int64| является положительным уникальным идентификатором пользователя | Не является обезательным полем|
|sort_by | string| в данном параметре необходимо указать по какому полю провести сортировку, сейчас доступны два поля created_at(дата создания) и amount(сумма операции)| Не является обезательным полем |
|order_by | string| в данном поле необходимо указать в каком порядке необходимо сделать сортировку, по умолчанию всегда указан ASC(от меньшего к большему), можно указать desc(от большего к меньшему) | Не является обезательным полем |
